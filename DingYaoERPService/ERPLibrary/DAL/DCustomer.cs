...
namespace DingYaoERP.DAL
{
    ...
    public class DCustomer
    {
        ...
        public string Add(Models.MCustomer mod)
        {
            ...
            cmd.Parameters.Add("@HugeSearch", SqlDbType.Bit).Value = mod.HugeSearch;
        ...
        private Models.MCustomer SetModel(SqlDataReader dr)
        {
            ...
            while (dr.Read())
            {
                ...
                mod.HugeSearch = bool.Parse(dr["HugeSearch"].ToString());
        ...
        /// <param name="strCustomerID"></param>
        /// <returns></returns>
        public List<Models.MCustomer> GetListByPriceGroupIDAndKeyword(string strPriceGroupID, string strKeyword)
        {
            SqlCommand cmd = new SqlCommand();
            StringBuilder sbTSQL = new StringBuilder();
            cmd.CommandType = CommandType.Text;
            sbTSQL.Append(" select * from TB_Customer where PriceGroupID=@PriceGroupID  ");
            cmd.Parameters.Add("@PriceGroupID", SqlDbType.NVarChar).Value = strPriceGroupID;

            if (strKeyword.Length > 0)
            {
                sbTSQL.Append(" and (CustomerID like @Keyword or CustomerName like @Keyword or CustomerKeyboard like @Keyword )");
